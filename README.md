...
- 结果查看：结果页展示目标品牌提及率、竞品提及率、平均排名、Top1/Top3 推荐率排行、各平台表现、问题列表、引用信源、回答详情和截图入口。
...
- 文心一言 CDP 模式会避免直接篡改富文本 DOM，改用真实点击、清空和逐字键盘输入；提交前会检测问题文本是否被重复写入，重复时会清空后重输一次。如果提交后页面提示“输入内容为空”，会自动用同样的键盘输入流程重试一次，并记录页面诊断。只有检测到明确的人机验证/风控文案时才标记为 `blocked`。
- 文心一言出现“开始全网搜索”等中间状态时，会继续等待最终回答，避免搜索刚开始就截图保存。
- 豆包和文心一言 CDP 模式会在提交前轮询等待聊天输入框完成渲染，降低 SPA 页面初始化较慢造成的偶发 `Input selector was not visible` 失败。
- 引用信源采集会在引用面板中执行初始采集加 5 轮滚动采集，减少通义千问、文心一言等平台因右侧来源列表未滚到底而漏采的情况。
- 通义千问引用信源会优先解析引用面板 DOM 中的 `ref_url` 字段；如果点击入口后页面已出现 `ref_url` 节点，会跳过二次点击并直接采集，且面板识别不再强依赖右侧可视位置，避免把 logo 链接计入引用，也避免面板布局压缩时漏采。
- 文心一言 CDP 模式打开引用列表后会逐个点击引用卡片，通过新标签或当前页跳转捕获真实外部 URL；如果某个引用卡片无法拿到 URL，会保留标题和站点信息但不进入 URL 引用统计。
